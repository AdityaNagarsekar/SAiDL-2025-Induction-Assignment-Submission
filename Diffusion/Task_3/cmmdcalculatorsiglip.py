"""
Calculate SigLIP and ALIGN Mean Maximum Discrepancy between two sets of images.

Uses features extracted from a SigLIP and ALIGN model and computes the MMD^2 score.
"""
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return image  # <<< RETURN PIL IMAGE INSTEAD
...
def collate_fn(batch):
    ...
    if not batch:
        return None # Still return None if the whole batch failed
    # Return the list of valid PIL Images directly
    return batch # <<< RETURN THE LIST, NOT torch.stack(batch)

@torch.no_grad()
def get_siglip_features(image_dir, model, processor, device, batch_size=64):
    ...
    all_features = []
    print_info(f"Extracting {model.__class__.__name__} features from {os.path.basename(image_dir)}...")
    for batch in tqdm(dataloader, desc=f"Feature Extraction ({os.path.basename(image_dir)})"):
        # 'batch' is now a list of PIL Images (or None if the whole batch failed)
        if batch is None:
            print_warning("Skipping an empty/failed batch returned by DataLoader.")
            continue
        if not batch:  # Double check in case collate_fn returns an empty list (shouldn't happen with check inside)
            print_warning("Skipping an empty batch list.")
            continue

        try:
            # Process the list of PIL images directly
...
def main():
    ...
    try:
        ...
        model = AutoModel.from_pretrained(args.model_id).to(args.device)
        model.eval()
    except Exception as e:
        print_error(f"Failed to load model or processor '{args.model_id}': {e}")
        sys.exit(1)
    ...
    try:
        real_features = get_siglip_features(args.real_path, model, processor, args.device, args.batch_size)
        fake_features = get_siglip_features(args.fake_path, model, processor, args.device, args.batch_size)
    except Exception as e:
